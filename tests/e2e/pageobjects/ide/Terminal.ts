...
export class Terminal {
    ...
    async selectTabByPreffixAndWaitText(terminalTab: string, expectedText: string, timeout: number = TestConstants.TS_SELENIUM_DEFAULT_TIMEOUT) {
        Logger.debug(`Terminal.selectTabByPreffixAndWaitText tab: ${terminalTab} text: ${expectedText}`);

        const terminalTabLocatorWithPreffix: string = `//li[contains(@title, '${terminalTab}')]`;
        const terminalIndex: number = await this.getTerminalIndex(terminalTab);
        const focusedTerminalTabLocator: string = terminalTabLocatorWithPreffix + '[@class=\'p-TabBar-tab p-mod-closable p-mod-current\']';
        
        await this.driverHelper.waitAndClick(By.xpath(terminalTabLocatorWithPreffix), timeout);
        await this.driverHelper.waitPresence(By.xpath(focusedTerminalTabLocator), timeout);
